...
Phase 3 管理窗口通过 Tauri command 调用 Rust 服务的 protocol v1 管理 IPC。设置页覆盖全部用户配置；模型页提交本地 GGUF 路径并显示服务返回的文件大小、SHA-256 和加载状态；词库页导入/导出 JSON、清空并显示条目预览；诊断页只显示服务状态、配置 revision、模型状态、词库数量和最近 UI 操作。UI 不直接读写配置、模型或用户词库持久化文件。
